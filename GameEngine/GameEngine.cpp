...
void GameEngine::handleSetUp(std::string input)
{
    ...
    case State::START:
        ...
        {
            std::cout << "Input the parameters for the tournament: tournament -M <listofmapfiles>(1 to 5) -P <listofplayerstrategies>(2 to 4)" << "-G <numberofgames>(1 to 5) -D <maxnumberofturns>(10 to 50)\n";
    ...
    case State::PLAYERSADDED:
        if (input == "addplayer")
        {
            ...
            if (std::find_if(players.begin(), players.end(), [&](const Player &p)
                             { return p.getName() == playerName; }) != players.end())
            {
...
void GameEngine::printPlayerStats(int roundnum)
{
    ...
    for (int i = 0; i < players.size(); i++)
    {
        ...
        std::sort(ts.begin(), ts.end(), [](Territory *a, Territory *b)
...
void GameEngine::printTournamentResults()
{
    ...
    for (size_t i = 0; i < tournamentMaps.size(); i++)
    {
        ...
        if (i < tournamentMaps.size() - 1)
            table << ", ";
    }

    table << "\nP: ";
    for (size_t i = 0; i < tournamentStrategies.size(); i++)
    {
        table << tournamentStrategies[i];
        if (i < tournamentStrategies.size() - 1)
            table << ", ";
    }
