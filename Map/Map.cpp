...
bool Map::doesEachTerritoryBelongToOneContinent() const
{
    ...
    for (Territory *t : *allTerritories)
        territoryContinentCount[t] = 0;
    for (Continent *c : *allContinents)
    {
        for (Territory *t : *c->getTerritories())
        {
            if (territoryContinentCount.find(t) != territoryContinentCount.end())
                territoryContinentCount[t]++;
            else
                return false;
        }
    }
    for (auto const &[territory, count] : territoryContinentCount)
        if (count != 1)
            return false;
    return true;
}
...
void Map::traverseContinent(Territory *start, std::vector<bool> &visited, const Continent *continent) const
{
    ...
    for (Territory *neighbor : *start->getAdjacentTerritories())
    {
        if (neighbor->getContinent() == continent)
            traverseContinent(neighbor, visited, continent);
    }
}
